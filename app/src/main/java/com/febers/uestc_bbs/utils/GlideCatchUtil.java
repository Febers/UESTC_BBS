...
public class GlideCatchUtil {
    ...
    private final String imageExternalCatchDir = MyApplication.Companion.context().getExternalCacheDir() + "/image_cache";
        ...
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) {
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        Glide.get(MyApplication.Companion.context()).clearDiskCache();
                    }
                }).start();
            } else {
                Glide.get(MyApplication.Companion.context()).clearDiskCache();
    ...
    public boolean clearCacheMemory() {
        try {
            if (Looper.myLooper() == Looper.getMainLooper()) { //只能在主线程执行
                Glide.get(MyApplication.Companion.context()).clearMemory();
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
