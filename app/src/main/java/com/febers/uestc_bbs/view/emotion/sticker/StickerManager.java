package com.febers.uestc_bbs.view.emotion.sticker;

import android.util.Xml;

import com.febers.uestc_bbs.view.emotion.EmotionManager;
import com.febers.uestc_bbs.view.emotion.utils.ZipUtils;
...
public class StickerManager {
    ...
    public void loadStickerCategory() {
        if (EmotionManager.getInstance().getStickerPath() == null) {
            return;
        }
        stickerCategories.clear();
        stickerCategoryMap.clear();
        File stickerDir = new File(EmotionManager.getInstance().getStickerPath());
    ...
    public void addZipResource(String zipPath) {
        ZipUtils.unZipFiles(zipPath, EmotionManager.getInstance().getStickerPath());
        EmotionManager.getInstance().getManagedView().reloadEmos(0);
    }
    ...
    private void loadStickerData(StickerCategory category) {
        ...
        File stickerDir = new File(EmotionManager.getInstance().getStickerPath(), category.getName());
        if (stickerDir.exists()) {
        ...
        if (tmp != 0) {
            int tmp2 = EmotionManager.getInstance().getStickerPerPage() - (stickers.size() - (stickers.size() / stickerPerPage) * stickerPerPage);
    ...
    private class EntryLoader extends DefaultHandler {
        ...
        List<StickerItem> load(StickerCategory category) {
            mCategory = category;
            stickerPath = new File(EmotionManager.getInstance().getStickerPath(), category.getName()).getAbsolutePath();
            InputStream is = null;
            try {
