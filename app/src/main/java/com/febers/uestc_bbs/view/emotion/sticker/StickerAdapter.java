...
public class StickerAdapter extends BaseAdapter {
    ...
    public StickerAdapter(Context context, StickerCategory category, int emotionLayoutWidth, int emotionLayoutHeight, int startIndex) {
        ...
        float perWidth = emotionLayoutWidth * 1f / EmotionManager.getInstance().getStickerColumn();
        mPerHeight = realPagerHeight * 1f / EmotionManager.getInstance().getStickerRow();
    ...
    @Override
    public int getCount() {
        return EmotionManager.getInstance().getStickerPerPage();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (mCategory.isDefault() && index == 0) {
            ...
            if (stickerBitmapUri != null) {
                EmotionManager.getInstance().getIImageLoader().displayImage(stickerBitmapUri, viewHolder.mImageView);
            }
        }
        return convertView;
    }
