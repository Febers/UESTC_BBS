package com.febers.uestc_bbs.view.emotion.emoticons;
...
import com.febers.uestc_bbs.view.emotion.EmotionDirHelper;
...
public class EmoticonManager {
    ...
    private EmoticonManager() {
        mEmotionManager = EmotionManager.getInstance();
        loadEmoticons();
    }
    ...
    private void loadEmoticons() {
        load(mEmotionManager.getContext(), mEmotionManager.getEmotDir() + File.separator + mEmotionManager.getConfigFile());
    ...
    private void load(Context context, String xmlPath) {
        ...
        //补充最后一页少的表情,空白占位
        int tmp = mDefaultEntries.size() % mEmotionManager.getEmojiPerPage();
        if (tmp != 0) {
            int tmp2 = mEmotionManager.getEmojiPerPage() - (mDefaultEntries.size() - (mDefaultEntries.size()
                    / mEmotionManager.getEmojiPerPage()) * mEmotionManager.getEmojiPerPage());
    ...
    private class EntryLoader extends DefaultHandler {
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
            if (localName.equals("Catalog")) {
                ...
                ImageEntry entry = new ImageEntry(tag, EmotionManager.getInstance().getEmotDir() + File.separator + catalog + File.separator + fileName);
                mText2Entry.put(entry.text, entry);
                if (catalog.equals(EmotionManager.getInstance().getSourceDir())) {
    ...
    private boolean isFileExists(String filename) {
        AssetManager assetManager = mEmotionManager.getContext().getAssets();
        try {
            String[] names = assetManager.list(mEmotionManager.getEmotDir() + File.separator + mEmotionManager.getSourceDir());
            for (String name : names) {
                String sourceName = mEmotionManager.getEmotDir() + File.separator + mEmotionManager.getSourceDir() + File.separator + name;
