...
import com.febers.uestc_bbs.view.emotion.emoticons.gif.AnimatedGifDrawable;
import com.febers.uestc_bbs.view.emotion.emoticons.gif.AnimatedImageSpan;
import com.febers.uestc_bbs.view.emotion.emoticons.gif.GifRunnable;
import com.febers.uestc_bbs.view.emotion.utils.EmoticonUtils;
...
public class EmotionTranslator {
    ...
    private static EmotionTranslator sEmoTranslator;
    ...
    public static EmotionTranslator getInstance() {
        if (sEmoTranslator == null) {
            synchronized (EmotionTranslator.class) {
                if (sEmoTranslator == null) {
                    sEmoTranslator = new EmotionTranslator();
                }
            }
        }
        sEmoTranslator.MAX_PER_VIEW = EmotionManager.getInstance().getMaxGifPerView();
     ...
     */
    public EmotionTranslator setMaxGifPerView(int maxGifPerView) {
    ...
    public SpannableString makeGifSpannable(String classTag, String value,
        ...
        Matcher matcher = EmotionManager.getInstance().getPattern().matcher(value);
        /*
        ...
        */
        ...
        for (int[] faceIndex : faceInfo) {
            if (faces > MAX_PER_VIEW) {
                Drawable drawable = getEmotDrawable(EmotionManager.getInstance().getContext(), value.substring(faceIndex[0], faceIndex[1]));
                if (drawable != null) {
                    ImageSpan span = new ImageSpan(drawable);
                    spannableString.setSpan(span, faceIndex[0], faceIndex[1], Spannable.SPAN_INCLUSIVE_INCLUSIVE);
                }
            } else {
                AnimatedGifDrawable gifDrawable = EmoticonManager.getInstance().getDrawableGif(EmotionManager.getInstance().getContext(),
    ...
    public SpannableString makeEmojiSpannable(String value) {
        ...
        while (matcher.find()) {
            ...
            Drawable drawable = getEmotDrawable(EmotionManager.getInstance().getContext(), emot);
            if (drawable != null) {
                ImageSpan span = new ImageSpan(drawable);
    ...
    public void replaceEmoticons(Editable editable, int start, int count) {
        ...
        Matcher matcher = EmotionManager.getInstance().getPattern().matcher(s);
        while (matcher.find()) {
            int from = start + matcher.start();
            int to = start + matcher.end();
            String emot = editable.subSequence(from, to).toString();
            Drawable d = getEmotDrawable(EmotionManager.getInstance().getContext(), emot);
            if (d != null) {
    ...
    private Drawable getEmotDrawable(Context context, String text) {
        ...
        int size = EmoticonUtils.dp2px(context, EmotionManager.getInstance().getDefaultEmoBoundsDp());
        if (drawable != null) {
